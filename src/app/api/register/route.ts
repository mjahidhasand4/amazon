    ...
    if ( name && (isEmail || (isMobile && countryCode)) && `${password}`.length >= 6) {
      ...
      if (rows.length > 0) {
        ...
        if ( (isEmail && isEmailVerified) || (isMobile && isMobilePhoneNumberVerified)) {
          return NextResponse.json({ error: "Registration failed", message: `There's already an account with this ${(isEmail && "email") || (isMobile && "mobile phone number")}.` }, { status: 400 });
        }

        const existingUserId = rows[0].id;
        await sql`UPDATE USERS SET otp_code=${OTP} WHERE id=${existingUserId}`;
        const verify_token = jwt.sign( { id: existingUserId, type: isEmailOrMobile }, process.env.JWT_ACCESS_TOKEN_SECRET!);
      ...
      const newUser = await sql`INSERT INTO USERS (name, password, country_code, email, mobile_phone_number, otp_code) VALUES (${name}, ${hashedPassword}, ${countryCode}, ${isEmail ? contact : ""}, ${isMobile ? contact : ""}, ${OTP}) RETURNING id;`;
      const newUserId = newUser.rows[0].id;
      const verify_token = jwt.sign( { id: newUserId, type: isEmailOrMobile }, process.env.JWT_ACCESS_TOKEN_SECRET!);
